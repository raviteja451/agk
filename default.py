...
import urllib, urllib2,cgi, xbmc,xbmcplugin, xbmcaddon, xbmcgui, os, random, re, json
import cookielib
import time
...
def addDir(name, url, mode, iconimage, catid, fanart=False, infoLabels=False):
    u = sys.argv[0] + "?url=" + urllib.quote_plus(url) + "&mode=" + str(mode) + "&name=" + urllib.quote_plus(name) + "&catid=" + str(catid)
    ok = True
